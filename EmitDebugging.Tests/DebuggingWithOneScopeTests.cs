...
{
	public sealed class DebuggingWithOneScopeTests
		: DebuggingTests
	{
		...
		protected override void AssertSequencePoints(XPathNavigator navigator)
		{
			var entries = navigator.Select(
				"./symbols/methods/method[@name=\"MethodWithOneScope.SimpleClass.ContainsOneScope\"]/sequencepoints/entry");
		...
		{
			using (var assembly = DebuggingTests.CreateDebuggingAssembly("MethodWithOneScope"))
			{
					...
					"SimpleClass"))
				{
					...
					using (var method = type.GetMethodDebugging(
						type.Builder.DefineMethod("ContainsOneScope",
		...
		protected override string[] GetExpectedFileLines()
		{
			return new string[] 
			{
				".assembly MethodWithOneScope",
				"{",
				"	.class public sealed ansi MethodWithOneScope.SimpleClass extends object",
				"	{",
				...
				"		}",
				"		",
				"		.method public hidebysig instance void ContainsOneScope() cil managed",
				"		{",
