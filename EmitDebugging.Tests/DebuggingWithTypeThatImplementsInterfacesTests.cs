...
{
	public sealed class DebuggingWithTypeThatImplementsInterfacesTests
		: DebuggingTests
	{
		...
		protected override void AssertSequencePoints(XPathNavigator navigator)
		{
			var entries = navigator.Select(
				"./symbols/methods/method[@name=\"AssemblyWithOneTypeThatImplementsInterfaces.SimpleClass.JustReturn\"]/sequencepoints/entry");
		...
		{
			using (var assembly = DebuggingTests.CreateDebuggingAssembly("AssemblyWithOneTypeThatImplementsInterfaces"))
			{
				using (var type = DebuggingTests.CreateDebuggingType(assembly,
					...
					"SimpleClass", new HashSet<Type>() { typeof(IComparable), typeof(IDisposable) }))
				{
		...
		protected override string[] GetExpectedFileLines()
		{
			return new string[] 
			{
				".assembly AssemblyWithOneTypeThatImplementsInterfaces",
				"{",
				"	.class public sealed ansi AssemblyWithOneTypeThatImplementsInterfaces.SimpleClass extends object implements System.IComparable implements System.IDisposable",
				"	{",
